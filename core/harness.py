...
class Harness:
    ...
    def _compact_context(self, messages: list):
        ...
        1. Memory flush — save key findings before discarding context
        2. Summarize old messages via LLM (or fallback to truncation)
    ...
    def _memory_flush(self, messages: list):
        """Flush key findings to persistent storage before compaction."""
        from core.config import get
    ...
    def _load_prior_context(self) -> str:
        ...
        if not get("cross_session.enabled", True):
            return ""
        ...
        try:
            ...
            if obs:
                ...
                for o in obs[-max_obs:]:
                    lines.append(
                        f"- [{o.id}] {o.subject}: {o.claim} "
                        f"(source: {o.source}, relevance: {o.relevance})"
                    )
                sections.append("\n".join(lines))
        except Exception:
            pass
